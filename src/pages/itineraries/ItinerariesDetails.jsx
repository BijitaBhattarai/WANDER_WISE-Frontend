...
const ItinerariesDetails = () => {
  ...
  if (itineraryLoading) return <Loading />;
  return (
              ...
              <Card key={index}>
                <CardContent className="text-center py-4 space-y-2">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      navigate(
                        `/itineraries/${id}/add?date=${thisDate.toISOString().split("T")[0]}`,
                      );
                    }}
                  >
                    <Plus /> Add Itinerary
            ...
            <Card className="bg-blue-100">
              <CardHeader>
                ...
                <CardAction className="flex gap-2 items-center">
                  <span>{formatDate(currentItinerary.date)}</span>
                  <Button
                    variant="outline"
                    onClick={() => {
                      navigate(
                        `/itineraries/${id}/edit/${currentItinerary._id}`,
                      );
                    }}
                  >
                    Edit Itinerary
